...
import { buildSearchQueryString } from '../utils/searchUtils'
...
function Home() {
    ...
    const handleSearch = (criteria) => {
        const query = buildSearchQueryString(criteria)
        navigate(query ? `/hospedagem?${query}` : '/hospedagem')
    }
                ...
                <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
                    <div className="relative z-10">
                        ...
                        <div className="flex flex-wrap gap-6">
                            ...
                            <Link to="/login" className={`px-10 py-5 font-black rounded-2xl transition backdrop-blur-md ${t.secondaryBtn}`}>
                                Entrar
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 relative">
                        <div className="space-y-4 pt-12">
                            <img src="https://images.unsplash.com/photo-1721029145366-205b3b927b3d?q=80&w=1528&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" className={`rounded-[2.5rem] h-80 w-full object-cover ${t.imageBorder}`} alt="Marau" />
                            <img src="https://images.unsplash.com/photo-1590523277543-a94d2e4eb00b?auto=format&fit=crop&q=80" className={`rounded-[2.5rem] h-56 w-full object-cover ${t.imageBorder}`} alt="Bahia" />
                        </div>
                        <div className="space-y-4">
                            <img src="https://images.unsplash.com/photo-1571003123894-1f0594d2b5d9?auto=format&fit=crop&q=80" className={`rounded-[2.5rem] h-56 w-full object-cover ${t.imageBorder}`} alt="Hotel" />
                            <img src="https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?auto=format&fit=crop&q=80" className={`rounded-[2.5rem] h-80 w-full object-cover ${t.imageBorder}`} alt="Resort" />
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main id="destinos" className="max-w-7xl mx-auto px-6 py-32">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                    <div className="max-w-lg">
                        ...
                        <p className={`${t.mutedSoft} font-medium`}>Quartos cadastrados pelo host, prontos para reserva pelo cliente.</p>
                    </div>
                    <Link to="/hospedagem" className={`text-amber-400 font-black text-xs uppercase tracking-widest border-b-2 border-amber-400 pb-1 transition ${t.linkAccent}`}>
                        ...
                        <button key={item.id} onClick={() => openAccommodation(item)} className="group cursor-pointer text-left">
                            ...
                            <div className="flex justify-between items-start gap-4">
                                <div>
                                    ...
                                    <p className={`${t.mutedSoft} text-sm font-medium`}>{item.location}</p>
                                </div>
                                <div className="text-right">
                                    <span className={`text-xl font-black ${t.heading}`}>R$ {Number(item.price || 0).toLocaleString('pt-BR')}</span>
                ...
                <section id="sobre" className={`mt-40 p-10 md:p-20 rounded-[4rem] relative overflow-hidden ${t.sectionCard}`}>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-10">
                                <div>
                                    ...
                                    <p className={`text-sm font-bold ${t.body}`}>React & Tailwind</p>
                                </div>
                                <div>
                                    <h4 className="text-amber-400 font-black text-xs uppercase tracking-[0.2em] mb-3">Back-end</h4>
                                    <p className={`text-sm font-bold ${t.body}`}>Spring Boot</p>
                                </div>
                                <div>
                                    <h4 className="text-amber-400 font-black text-xs uppercase tracking-[0.2em] mb-3">Banco</h4>
                                    <p className={`text-sm font-bold ${t.body}`}>MySQL</p>
                                </div>
                            </div>
                        </div>

                        <div className={`p-10 rounded-[3rem] backdrop-blur-xl ${t.panel}`}>
                            <h4 className={`font-black text-xs uppercase tracking-[0.3em] mb-8 flex items-center gap-3 ${t.heading}`}>
                            ...
                            <div className="space-y-6">
                                {[
                                    ...
                                    <div key={nome} className="flex items-center gap-4 group cursor-default">
                                        ...
                                        <span className={`font-bold group-hover:text-amber-500 transition ${t.body}`}>
                                            {nome}
                                        </span>
                                    </div>
                                ))}
